...
module.exports = function setupWebhooks(platform) {
	...
	for (const wh of platform.webhooks) {
		...
		if (!wh.token) {
			...
			// Token impostato manualmente in config: sincronizza nel contesto se differente
			if (ctx.token !== wh.token) {
				ctx.token = wh.token;
				// Se proviene da config lo consideriamo non-autogenerato
				ctx.tokenAutogenerated = !!wh._autogenerated;
				accessory.context = ctx;
				try { platform.api.updatePlatformAccessories([accessory]); } catch (_) {}
			}
		}
