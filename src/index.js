...
class ProtectWebhookPlatform {
	...
	_setupServer() {
		...
		app.use((req, res, next) => {
			const redactedUrl = this._redactUrl(req.url);
			this.log.debug(`HTTP ${req.method} ${redactedUrl} from ${this._getRemoteIp(req)}`);
			next();
		});
		...
		this.server = app.listen(this.port, this.bindAddress, () => {
			this.log.info(`Webhook server listening on ${this._serverBaseUrl()} (local-only=${this.enforceLocalOnly})`);
		});
		...
		this.app = app;
			...
			app.get("/admin/webhooks/:name/token", (req, res) => {
				...
				if (!wh) return res.status(404).json({ error: "not found" });
					...
					const url = this._composeWebhookUrl(req, wh);
			...
			app.get("/admin/webhooks", (req, res) => {
				if (!this._isLocalAdminRequest(req)) return res.status(403).json({ error: "forbidden" });
				const list = this.webhooks.map((w) => ({ name: w.name, path: this._computeWebhookPath(w) }));
		...
		for (const wh of this.webhooks) {
			...
			const ctx = accessory.context || {};
				...
				if (ctx.token) {
					...
					this.api.updatePlatformAccessories([accessory]);
							// Intenzionalmente non logghiamo il token; l'utente può recuperarlo via endpoint admin.
				}
			}
			...
			this.log.info(`Webhook '${wh.name}' ready at ${this._serverBaseUrl()}${fullPath} (token via header x-webhook-token or ?token=...)`);
		}
	}
